...
export class EditPlaylistComponent implements OnInit {
  ...
  editPlaylist(){
    ...
    if(this.playlistForm.get('title')?.value&&this.selected.length>0&&token){
      if(this.mode == 'create'){
        this.playlistService.addPlaylist({jwt_token : token,
          name : this.playlistForm.get('title')!.value as string, 
       description : this.playlistForm.get('description')!.value as string ,
       tags : this.selected}).then(res=>{
         this.dialogRef.close('refresh')
       })
      }
      if(this.mode =='edit'){
        this.playlistService.updatePlaylist({jwt_token : token, playlist_id : this.playlistId as number,
          name : this.playlistForm.get('title')!.value as string, 
       description : this.playlistForm.get('description')!.value as string ,
       tags : this.selected}).then(res=>{
         this.dialogRef.close('refresh')
